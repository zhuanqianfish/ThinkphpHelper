...
function fishAutoLoad($className){
    require_once $className.'.php';
}
spl_autoload_register("Core\\WY\\fishAutoLoad", true);
...
class Template
{
     ...
     * @param  array $config
     */
    public function __construct(array $config = [], string $module='')
    {
        $this->config['cache_path'] = 'temp/';
        ...
        $this->storage =  new \Core\WY\template\driver\File;
    ...
    public function parseTagLib($tagLib, &$content, $hide = false)
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\Core\\WY\\template\\taglib\\' . ucwords($tagLib);
        }
    ...
    private function parseTemplateFile($template)
    {
       ...
       // if ('' == pathinfo($template, PATHINFO_EXTENSION)) {  //这里暂时注释掉
            if (strpos($template, '@')) {
